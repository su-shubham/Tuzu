...
async def insert_members(email:str,password_hash:str,db:Connection) -> Members:
    result = await db.fetch_one(
        '''
...
async def update_member(id:int,password_hash:str,db:Connection) -> None:
...
async def update_member_email_timestamp(id:int,db:Connection) -> None:
    result = await db.execute(
        '''
        UPDATE members 
